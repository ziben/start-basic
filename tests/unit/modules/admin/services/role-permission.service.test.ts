...
const { mockRolePermission, mockRole, mockTransaction } = vi.hoisted(() => ({
  mockRolePermission: {
    findMany: vi.fn(),
    deleteMany: vi.fn(),
    createMany: vi.fn(),
...
describe('RolePermissionService', () => {
  ...
  describe('assignPermissions', () => {
    it('应该为角色分配权限', async () => {
      mockRole.findUnique.mockResolvedValue({ id: 'role1', name: 'admin' })
      ...
      const permissions = [{ permissionId: 'perm1', dataScope: 'ORG' }, { permissionId: 'perm2' }]

      const result = await RolePermissionService.assignPermissions('role1', permissions)

      expect(result).toEqual({ success: true })
      expect(mockRole.findUnique).toHaveBeenCalledWith({ where: { id: 'role1' } })
    })

    it('角色不存在时应该抛出错误', async () => {
      mockRole.findUnique.mockResolvedValue(null)

      await expect(RolePermissionService.assignPermissions('nonexistent', [])).rejects.toThrow('角色不存在')
    })

    it('应该使用默认的 dataScope', async () => {
      mockRole.findUnique.mockResolvedValue({ id: 'role1' })
      ...
      await RolePermissionService.assignPermissions('role1', [{ permissionId: 'perm1' }])
