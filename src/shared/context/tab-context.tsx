...
export function TabProvider({ children }: { children: React.ReactNode }): React.ReactElement {
  ...
  const currentScope = useMemo((): TabScope => {
    return getTabScope(location.pathname)
  }, [location.pathname])
  ...
  const openTab = useCallback(
    (path: string | undefined, title: string, icon?: React.ElementType | string): boolean => {
      ...
      const targetScope = getTabScope(normalizedPath)
      const existingTab = tabs.find((tab) => tab.path === normalizedPath && tab.scope === targetScope)
      ...
      if (existingTab) {
        if (existingTab.title !== title || existingTab.icon !== icon) {
          setTabs((prev) =>
            dedupeTabs(
              prev.map((tab) =>
                tab.path === normalizedPath && tab.scope === targetScope ? { ...tab, title, icon } : tab
              )
            )
          )
        }
        if (normalizeTabPath(location.pathname) !== normalizedPath) {
          navigate({ to: normalizedPath as NavigateTo })
        }
        return true
      }
      ...
      setTabs((prev) => {
        if (prev.some((tab) => tab.path === normalizedPath && tab.scope === targetScope)) {
          return dedupeTabs(prev)
        }
        return dedupeTabs([...prev, newTab])
      })
      if (normalizeTabPath(location.pathname) !== normalizedPath) {
        navigate({ to: normalizedPath as NavigateTo })
      }
  ...
  useEffect(() => {
    ...
    // 只处理对应的 Scope
    const targetScope = getTabScope(path)
    if (targetScope !== currentScope) return

    const title = getTitleForPath(path)
    const existingTab = tabs.find((tab) => tab.path === path && tab.scope === targetScope)
    if (existingTab) {
      if (existingTab.title === title) return

      const frame = requestAnimationFrame(() => {
        setTabs((prev) =>
          dedupeTabs(prev.map((tab) => (tab.path === path && tab.scope === targetScope ? { ...tab, title } : tab)))
        )
      })
  ...
  useEffect(() => {
    ...
    try {
      ...
      frame = requestAnimationFrame(() => setTabs(dedupeTabs(merged)))
    } catch {
      frame = requestAnimationFrame(() => setTabs(getDefaultTabs()))
    }
  ...
  return (
    <TabStateContext.Provider value={stateValue}>
      <TabActionContext.Provider value={actionValue}>{children}</TabActionContext.Provider>
