...
export async function handleQueryOrderStatus(orderId: string, headers: Headers): Promise<PaymentOrderStatusResult> {
...
export async function handleSyncOrderStatus(orderId: string, headers: Headers): Promise<SyncPaymentOrderStatusResult> {
  const { getWeChatPayClient } = await import('../lib/wechat-pay')
...
export async function handleCloseOrder(orderId: string, headers: Headers): Promise<ClosePaymentOrderResult> {
  const { getWeChatPayClient } = await import('../lib/wechat-pay')
  const { sessionUserId, prisma } = await getPaymentRequestContext(headers)
  const wechatPayClient = await getWeChatPayClient()
