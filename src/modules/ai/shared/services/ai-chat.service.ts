...
export class AiChatService {
   ...
   * 获取用户的对话列表
   */
  static async listConversations(userId: string, page = 1) {
  ...
  static async adminListConversations(
    input: {
      page?: number
      pageSize?: number
      filter?: string
    ...
    const orderBy: Prisma.AIConversationOrderByWithRelationInput =
      sortBy === 'title' || sortBy === 'createdAt' ? { [sortBy]: sortDir ?? 'desc' } : { updatedAt: 'desc' }
  ...
  static async adminGetConversation(conversationId: string) {
    ...
    const totalTokens = conversation.messages.reduce((sum, msg) => sum + (msg.totalTokens ?? 0), 0)
