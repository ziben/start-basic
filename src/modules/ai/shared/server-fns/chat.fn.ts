...
export const chatInputSchema = z.object({
  ...
  modelProvider: z.enum(['openai', 'gemini', 'deepseek', 'qwen', 'zhipu', 'ernie']).optional(),
 ...
 * 提供基本的请求流式响应
 */
export async function handleChat({ data }: { data: ChatInput }) {
  await requireUser()
  try {
...
export const chatFn = createServerFn({ method: 'POST' }).validator(chatInputSchema).handler(handleChat)
