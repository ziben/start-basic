...
function ValueInput({
  ...
  if (valueType === 'JSON' || valueType === 'STRING_ARRAY') {
    const placeholder = valueType === 'JSON' ? '{"key": "value"}' : '["item1", "item2"]（每行一个，或 JSON 数组格式）'
    return (
      <Textarea
        value={value}
      ...
      <DialogContent className='sm:max-w-lg'>
        ...
        <Form {...form}>
          <form
            ...
            className='max-h-[60vh] space-y-4 overflow-y-auto px-0.5 py-1'
          >
            {/* Key */}
            <FormField
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='grid grid-cols-6 items-center space-y-0 gap-x-4 gap-y-1'>
                  ...
                  <FormControl>
                    <Input placeholder='system' className='col-span-4' autoComplete='off' {...field} />
                  </FormControl>
                  <FormMessage className='col-span-4 col-start-3 text-xs' />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='grid grid-cols-6 items-start space-y-0 gap-x-4 gap-y-1'>
                  ...
                  <div className='col-span-4'>
                    <FormControl>
                      <ValueInput valueType={valueType} value={field.value} onChange={field.onChange} />
                    </FormControl>
                  </div>
                  <FormMessage className='col-span-4 col-start-3 text-xs' />
                </FormItem>
            ...
            <div className='grid grid-cols-6 gap-x-4 gap-y-3'>
              <span className='col-span-2 self-start pt-1 text-end text-sm text-muted-foreground'>选项</span>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className='flex items-center gap-3 space-y-0'>
                      ...
                      <FormLabel className='cursor-pointer font-normal'>启用</FormLabel>
                    </FormItem>
                  )}
                />
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className='flex items-center gap-3 space-y-0'>
                      ...
                      <FormLabel className='cursor-pointer font-normal'>公开（前端可读）</FormLabel>
                    </FormItem>
                  )}
                />
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className='flex items-center gap-3 space-y-0'>
                      ...
                      <FormLabel className='cursor-pointer font-normal'>敏感值（界面遮盖）</FormLabel>
                    </FormItem>
                  )}
                />
              </div>
        ...
        <DialogFooter>
          ...
          <Button type='submit' form='system-config-create-form' disabled={createMutation.isPending}>
