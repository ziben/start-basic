...
type TranslationsImportDialogProps = {
  readonly open: boolean
  readonly onOpenChange: (open: boolean) => void
}

export function TranslationsImportDialog({ open, onOpenChange }: TranslationsImportDialogProps) {
    ...
    >
      <DialogContent className='gap-2 sm:max-w-sm'>
        <DialogHeader className='text-start'>
          <DialogTitle>{t('admin.navgroup.import.title') || 'Import Translations'}</DialogTitle>
          <DialogDescription>
            {t('admin.navgroup.import.desc') || 'Import translations quickly from a CSV file.'}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
