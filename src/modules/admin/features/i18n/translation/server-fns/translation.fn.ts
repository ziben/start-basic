 ...
 * [迁移自 admin/shared/server-fns/translation.fn.ts]
 */
import { z } from 'zod'
...
export const getTranslationsFn = createServerFn({ method: 'GET' })
  .validator(z.object({ locale: z.string().min(1).optional() }).optional())
...
export const importTranslationsFn = createServerFn({ method: 'POST' })
  .validator(z.array(z.object({ locale: z.string().min(1), key: z.string().min(1), value: z.string() })))
