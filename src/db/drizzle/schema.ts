...
export const navItem = sqliteTable('nav_item', {
  ...
  isCollapsible: integer({ mode: 'boolean' }).default(false).notNull(),
...
export const userRoleNavGroup = sqliteTable(
  'user_role_nav_group',
  {
    id: text().primaryKey(),
    userId: text()
    ...
    navGroupId: text()
      .notNull()
      .references(() => navGroup.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
    visible: integer({ mode: 'boolean' }).default(true).notNull(),
    createdAt: text().default(`CURRENT_TIMESTAMP`).notNull(),
  },
...
export const systemLog = sqliteTable(
  ...
  (table) => [
    index('system_log_userId_idx').on(table.userId),
...
export const qbTag = sqliteTable(
  'qb_tag',
  {
    id: text().primaryKey(),
    name: text().notNull(),
...
export const qbQuestionTag = sqliteTable(
  'qb_question_tag',
  {
    questionId: text()
      .notNull()
      .references(() => qbQuestion.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
    tagId: text()
      .notNull()
      .references(() => qbTag.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
  },
...
export const qbPracticeAttempt = sqliteTable('qb_practice_attempt', {
  ...
  sessionId: text()
    .notNull()
    .references(() => qbPracticeSession.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
  questionId: text()
    .notNull()
    .references(() => qbQuestion.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
...
export const departments = sqliteTable(
  ...
  (table) => [
    uniqueIndex('departments_organizationId_code_key').on(table.organizationId, table.code),
    index('departments_parentId_idx').on(table.parentId),
    index('departments_organizationId_idx').on(table.organizationId),
  ]
)

export const fieldPermissions = sqliteTable(
  'field_permissions',
  {
    id: text().primaryKey(),
    permissionId: text()
  ...
  (table) => [
    uniqueIndex('field_permissions_permissionId_resource_field_key').on(
      table.permissionId,
      table.resource,
      table.field
    ),
...
export const crossOrgAccess = sqliteTable(
  'cross_org_access',
  {
    id: text().primaryKey(),
    role: text().notNull(),
  ...
  (table) => [
    uniqueIndex('cross_org_access_role_sourceOrgId_targetOrgId_resource_key').on(
      table.role,
      table.sourceOrgId,
      table.targetOrgId,
      table.resource
...
export const roleNavGroup = sqliteTable(
  'role_nav_group',
  {
    id: text().primaryKey(),
    role: text().notNull(),
...
export const teamMember = sqliteTable(
  ...
  (table) => [index('teamMember_userId_idx').on(table.userId), index('teamMember_teamId_idx').on(table.teamId)]
)
...
export const roles = sqliteTable(
  'roles',
  {
    ...
    description: text(),
    scope: text().notNull(),
    isSystem: integer({ mode: 'boolean' }).default(false).notNull(),
    isTemplate: integer({ mode: 'boolean' }).default(false).notNull(),
  ...
  (table) => [
    index('roles_sortOrder_idx').on(table.sortOrder),
...
export const resources = sqliteTable(
  ...
  (table) => [
    index('resources_isSystem_idx').on(table.isSystem),
...
export const actions = sqliteTable(
  ...
  (table) => [
    uniqueIndex('actions_resourceId_name_key').on(table.resourceId, table.name),
...
export const permissions = sqliteTable(
  ...
  (table) => [
    uniqueIndex('permissions_resourceId_actionId_key').on(table.resourceId, table.actionId),
...
export const rolePermissions = sqliteTable(
  ...
  (table) => [
    uniqueIndex('role_permissions_roleId_permissionId_key').on(table.roleId, table.permissionId),
...
export const session = sqliteTable(
  ...
  (table) => [uniqueIndex('session_token_key').on(table.token), index('session_userId_idx').on(table.userId)]
)
...
export const organizationRolePermissions = sqliteTable(
  'organization_role_permissions',
  {
    id: text().primaryKey(),
    organizationRoleId: text()
      .notNull()
      .references(() => organizationRole.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
    permissionId: text()
      .notNull()
      .references(() => permissions.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
  ...
  (table) => [
    uniqueIndex('organization_role_permissions_organizationRoleId_permissionId_key').on(
...
export const organizationRole = sqliteTable(
  ...
  (table) => [
    uniqueIndex('organizationRole_organizationId_role_key').on(table.organizationId, table.role),
